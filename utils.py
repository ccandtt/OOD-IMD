...
def get_scheduler(config, optimizer):
    if config.sch is None or str(config.sch).lower() == 'none':
        print("[Scheduler] No scheduler is used.")
        return None

    assert config.sch in ['StepLR', 'MultiStepLR', 'ExponentialLR', 'CosineAnnealingLR', 'ReduceLROnPlateau',
                          'CosineAnnealingWarmRestarts', 'WP_MultiStepLR', 'WP_CosineLR'], f"Unsupported scheduler: {config.sch}"
    ...
    if config.sch == 'StepLR':
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=config.epochs,  # T_max is the maximum number of iterations
...
class PatchMiningLoss:
    def __init__(self, criterion):
        """
        参数:
            criterion: 主输出使用的损失函数，例如 nn.BCEWithLogitsLoss()
        """
        self.criterion = criterion

    def _weighted_loss(self, pred, mask):
        """
        加权 BCE + 加权 IoU 损失，用于 patch mining 输出。
        """
    ...
    def __call__(self, out, targets, patch_mining_list):
        """
        计算总损失，包括主输出损失和多层次 patch mining 损失。
        """
        # 主输出损失
        loss_main = self.criterion(out, targets)

        # Patch mining 分支损失（加权组合）
        weights = [1/2, 1/4, 1/8, 1/16]
        loss_patch = 0

        for i in range(4):
            patch_out = patch_mining_list[i]
            loss_patch += weights[i] * self._weighted_loss(patch_out, targets)

        total_loss = loss_main + loss_patch
        return total_loss
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        f = F.normalize(features, dim=1)                    # (B, C)
        sim = torch.matmul(f, f.T) / self.t                 # 余弦相似度
        B = labels.size(0)
        mask = labels.unsqueeze(1) == labels.unsqueeze(0)   # (B, B) Bool
        logits_max = sim.max(dim=1, keepdim=True)[0].detach()
        sim = sim - logits_max                              # 数值稳定
        exp_sim = torch.exp(sim) * (~torch.eye(B, dtype=torch.bool,
                                               device=features.device))
        log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True) + 1e-8)
        loss = -(log_prob * mask).sum() / (mask.sum() - B)  # 排除对角线
        return loss
...
class myResize:
    ...
    def __call__(self, data):
        if isinstance(data, tuple):
            ...
            return TF.resize(data, [self.size_h, self.size_w])
...
class myRandomRotation:
    ...
    def __call__(self, data):
        if isinstance(data, tuple):
            ...
            if random.random() < self.p:
                return TF.rotate(data, self.angle)
            else:
                return data
...
class myNormalize:
    ...
    def __call__(self, data):
        if isinstance(data, tuple):
            ...
            img_normalized = (data-self.mean)/self.std
            img_normalized = ((img_normalized - np.min(img_normalized)) 
                                / (np.max(img_normalized)-np.min(img_normalized))) * 255.
...
def cal_params_flops(model, size, logger):
    ...
    flops, params = profile(model, inputs=(input,), verbose=False)  ## 计算模型的FLOPs和参数量
